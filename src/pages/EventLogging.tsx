...
type ValidationErrors = { births?: string; deaths?: string; sales?: string };
...
const EventLogging = () => {
  ...
  const handleAdd = () => {
    const b = clamp(births); const d = clamp(deaths); const s = clamp(sales);
    const errs = validate(b, d, s);
    setErrors(errs);
    if (Object.keys(errs).length > 0) { toast.error("Fix validation errors before adding."); return; }
  ...
  const confirmEdit = () => {
    ...
    const b = clamp(editValues.births); const d = clamp(editValues.deaths); const s = clamp(editValues.sales);
    const errs = validate(b, d, s);
    setEditErrors(errs);
    if (Object.keys(errs).length > 0) { toast.error("Fix errors before saving."); return; }
    setRecords(prev => prev.map(r => r.year === editingYear ? { ...r, births: b, deaths: d, sales: s } : r));
    setEditingYear(null);
      ...
      <div className="max-w-5xl mx-auto px-6 py-10 space-y-10">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-display font-bold text-foreground mb-2 flex items-center gap-3">
              <BookOpen className="h-7 w-7 text-accent" /> Farm Ledger
            </h1>
            <p className="text-muted-foreground">Log yearly birth, death, and sales events for your herd.</p>
          </div>
          {records.length > 0 && (
            ...
            <div className="bg-card border border-border rounded-xl p-5">
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                <div className="space-y-1.5">
                  <Label className="text-xs font-medium text-muted-foreground flex items-center gap-1"><Calendar className="h-3 w-3" /> Year</Label>
                  <Select value={year.toString()} onValueChange={v => setYear(parseInt(v))}>
                    <SelectTrigger className="h-9"><SelectValue /></SelectTrigger>
                    <SelectContent>{availableYears.map(y => <SelectItem key={y} value={y.toString()}>Yr {y}</SelectItem>)}</SelectContent>
                  </Select>
                </div>
                <div className="space-y-1.5">
                  <Label className="text-xs font-medium text-muted-foreground flex items-center gap-1"><Baby className="h-3 w-3" /> Births</Label>
                  <Input type="number" min={0} max={MAX_COUNT} value={births}
                    onChange={e => { setBirths(parseInt(e.target.value) || 0); setErrors(prev => { const { births: _, ...rest } = prev; return rest; }); }}
                ...
                <div className="space-y-1.5">
                  <Label className="text-xs font-medium text-muted-foreground flex items-center gap-1"><HeartPulse className="h-3 w-3" /> Deaths</Label>
                  <Input type="number" min={0} max={MAX_COUNT} value={deaths}
                    onChange={e => { setDeaths(parseInt(e.target.value) || 0); setErrors(prev => { const { deaths: _, ...rest } = prev; return rest; }); }}
                ...
                <div className="space-y-1.5">
                  <Label className="text-xs font-medium text-muted-foreground flex items-center gap-1"><DollarSign className="h-3 w-3" /> Sales</Label>
                  <Input type="number" min={0} max={MAX_COUNT} value={sales}
                    onChange={e => { setSales(parseInt(e.target.value) || 0); setErrors(prev => { const { sales: _, ...rest } = prev; return rest; }); }}
              ...
              <div className="mt-4 flex justify-end">
                <Button onClick={handleAdd} size="sm" className="gap-1.5 px-6">
        ...
        {records.length > 0 ? (
                          ...
                          <>
                            ...
                            <TableCell className="text-right">
                              <div className="flex justify-end gap-1">
                                <Button size="icon" variant="ghost" className="h-7 w-7" onClick={confirmEdit}><Check className="h-3.5 w-3.5 text-primary" /></Button>
                                <Button size="icon" variant="ghost" className="h-7 w-7" onClick={cancelEdit}><X className="h-3.5 w-3.5 text-muted-foreground" /></Button>
                              </div>
                            </TableCell>
                          </>
                        ) : (
                          <>
                            ...
                            <TableCell className="text-right">
                              <div className="flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => startEdit(record)}><Pencil className="h-3 w-3" /></Button>
                                <Button size="icon" variant="ghost" className="h-7 w-7 hover:text-destructive" onClick={() => handleRemove(record.year)}><X className="h-3 w-3" /></Button>
                              </div>
                            </TableCell>
                          </>
                        )}
          ...
          <section className="text-center py-16 space-y-6">
            ...
            <div className="bg-card border border-dashed border-border rounded-xl p-5 max-w-lg mx-auto text-left space-y-3">
      ...
      <AlertDialog open={showClearDialog} onOpenChange={setShowClearDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>All {records.length} entries will be permanently removed.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
